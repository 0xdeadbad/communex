...
def test_module_server_call_success(
    serve, server_keypair: Keypair, my_keypair: Keypair
):
    ...
    assert result.json() == {
        "output": 'An answer example for the prompt "all good?"'
    }


def test_module_server_call_invalid_fn(
    serve, server_keypair: Keypair, my_keypair: Keypair
):
    server_ss58_address = check_ss58_address(server_keypair.ss58_address)
...
def test_module_server_call_blacklisted(
    serve, server_keypair: Keypair, some_module_server: ModuleServer
):
    server_ss58_address = check_ss58_address(server_keypair.ss58_address)
    ...
    assert result.json() == {
        "error": {"code": 403, "message": "You are blacklisted"}
    }


def test_module_server_call_not_blacklisted(
    serve, server_keypair: Keypair, some_module_server: ModuleServer
):
    server_ss58_address = check_ss58_address(server_keypair.ss58_address)
...
def test_module_server_call_whitelisted(
    serve,
    server_keypair: Keypair,
    my_keypair: Keypair,
    some_module_server: ModuleServer,
):
    server_ss58_address = check_ss58_address(server_keypair.ss58_address)
    ...
    assert result.status_code == 200
    assert result.json() == {
        "output": 'An answer example for the prompt "success?"'
    }


def test_module_server_call_not_whitelisted(
    serve,
    server_keypair: Keypair,
    my_keypair: Keypair,
    some_module_server: ModuleServer,
    ...
    assert result.status_code == 403
    assert result.json() == {
        "error": {"code": 403, "message": "You are not whitelisted"}
    }


def test_module_server_call_blacklist_overcomes_whitelist(
    serve,
    server_keypair: Keypair,
    my_keypair: Keypair,
    some_module_server: ModuleServer,
