...
def temporary_key_from_mnemonic(invoke_cli: InvokeCli):
    try:
        result = invoke_cli(
            ["key", "regen", TEST_TEMPORARY_KEY, TEST_FAKE_MNEM_DO_NOT_USE_THIS]
        )
...
def test_cli_key_create(temporary_key):
    ...
    assert (
        f"Key successfully stored with name '{TEST_TEMPORARY_KEY}'."
        in result.stdout
    )
...
def test_cli_key_save(invoke_cli: InvokeCli):
    try:
        ...
        assert (
            "`5EA6Dd3vejQco2FZomoAQgxacsTp7ZPFuR25TwxTiUKbkep1`."
            in result.stdout
        )
        assert (
            f"Key stored with name `{TEST_TEMPORARY_KEY}` successfully."
            in result.stdout
        )
    finally:
        delete_temporary_key()
...
def test_cli_key_show(invoke_cli: InvokeCli, temporary_key_from_mnemonic):
    ...
    assert (
        clean(
            "│ public_key    │ 5c93f7facb586801be42e9450646bc3183ac14d62b00c7036837d4d366c0… │"
        )
        in output
    )
...
def test_cli_key_show_private_option(
    invoke_cli: InvokeCli, temporary_key_from_mnemonic
):
    address, name, _ = temporary_key_from_mnemonic
    ...
    assert (
        clean(
            f"│ mnemonic     │ {TEST_FAKE_MNEM_DO_NOT_USE_THIS}                                 │"
        )
        in output
    )
    assert (
    ...
    assert (
        clean(
            "│ private_key   │ faf234e8bde612045e892a3312f5d03335805d7da2d617cec25947f47aa55d0be969db358ba3b741932379cce96e247849b70b0bc6968503fed7de28597b5483 │"
        )
        in output
    )
    assert (
        clean(
            f"│ ss58_address │ {address}                                                        │"
        )
        in output
    )
    assert (
        clean(
            "│ seed_hex      │ 890f730118632739aea624e1bebd76d557f6cd7b1779fa2507902d1a65265964 │"
        )
        in output
    )
    assert (
